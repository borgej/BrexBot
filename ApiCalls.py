...
class ApiCalls:
    ...
    def get_user_id(self, username=Config.CHANNEL_NAME):
        try:
            ...
            self.logger.error("Unable to retrieve the user ID for " + username)
    ...
    def get_user_data(self, username=Config.CHANNEL_NAME):
        try:
            ...
            self.logger.error("Unabe to retrieve the data for " + username)

    def get_channel_id(self, channel_name=Config.CHANNEL_NAME):
        try:
            return self.channel_id
        except Exception as e:
            self.logger.error("Could not retrieve channel ID for " + channel_name)
    ...
    def get_moderators(self):
        try:
            ...
            self.logger.error("Could not retrieve Moderator list for the channel")
    ...
    def is_moderator(self, viewer):
        try:
            ...
            moderator_data = self.json_data(url)
            if moderator_data == None:
                return False
            else:
                return True
        except Exception as e:
            self.logger.error("Unable to determin if " + viewer + " is a moderator.")
    ...
    def is_follower(self, viewer):
        try:
            ...
            self.logger.error("Unable to determin if " + viewer + " is following the channel.")
    ...
    def is_subscriber(self, viewer):
        try:
            ...
            self.logger.error("Unable to determin if " + viewer + " is subscribed to the channel.")
    ...
    def create_clip(self):
        try:
            ...
            self.logger.error("Couldn't create clip.")
    ...
    def is_banned(self, viewer):
        try:
            ...
            self.logger.error("Unable to check if " + viewer + " is banned from the channel.")
